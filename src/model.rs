...
impl Message {
	pub fn decode(mut value: BTreeMap<String, Value>) -> Result<Message> {
		let mut author = req!(into_map(req!(value.remove("author"))));
		Ok(Message {
			...
			author_id: UserId(req!(author.remove("id").and_then(into_string))),
			author_username: req!(author.remove("username").and_then(into_string)),
			author_discriminator: req!(author.remove("discriminator").and_then(into_string)),
			author_avatar: req!(author.remove("avatar").and_then(into_string)),
		})
	}
}
